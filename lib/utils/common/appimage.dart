class Appimage {
  static String logo = 'assets/images/Group 86.png';
  static String imageview1 = 'assets/images/Online Groceries-cuate 1.png';
  static String imageview2 = 'assets/images/Payment Information-cuate 1.png';
  static String imageview3 = 'assets/images/Take Away-pana 1.png';

  static String Applelogo = 'assets/images/apple-logo (1) 1.png';
  static String googlelogo = 'assets/images/search (1) 1.png';
  static String facebooklogo = 'assets/images/facebook (3) 1.png';
  static String homebtmcontainerimage =
      'assets/images/image-removebg-preview (67) 1.png';

  static List<String> gridviewimage = [
    'assets/images/image-removebg-preview (54) 1.png',
    'assets/images/image-removebg-preview (52) 1.png',
    'assets/images/image-removebg-preview (53) 1.png',
    'assets/images/image-removebg-preview (61) 1.png',
    'assets/images/image-removebg-preview (57) 2.png',
    'assets/images/image-removebg-preview (58) 1.png',
    'assets/images/image-removebg-preview (59) 1.png',
    'assets/images/image-removebg-preview (60) 1.png',
    'assets/images/image-removebg-preview (62) 1.png',
    'assets/images/image-removebg-preview (64) 1.png',
    'assets/images/image-removebg-preview (65) 1.png',
    'assets/images/image-removebg-preview (66) 1.png'
  ];
  static List<String> shopgridviewimage = [
    'assets/images/image-removebg-preview (84) 1.png',
    'assets/images/image-removebg-preview (85) 1.png',
    'assets/images/image-removebg-preview (86) 1.png',
    'assets/images/image-removebg-preview (89) 1.png'
  ];

  static String edit = 'assets/images/edit (3) 3.png';
  static String delete = 'assets/images/delete 1.png';
  static String officebag = 'assets/images/briefcase (2) 1.png';
  static String location = 'assets/images/location (4) 1.png';

}
